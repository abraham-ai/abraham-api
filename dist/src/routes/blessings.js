...
blessings.post("/", withAuth, async (c) => {
    try {
        ...
        const result = await blessingService.performBlessingOnchain(user.walletAddress, seedIdNum);
        if (!result.success) {
            ...
            if (result.error?.includes("not eligible"))
                ...
                statusCode = 404;
            else if (result.error?.includes("already blessed"))
                statusCode = 400;
...
blessings.post("/prepare", withAuth, async (c) => {
    try {
        ...
        const result = await blessingService.prepareBlessingTransaction(user.walletAddress, seedIdNum);
        if (!result.success) {
            // Determine appropriate status code based on error
            let statusCode = 500;
            if (result.error?.includes("not eligible"))
                ...
                statusCode = 400;
            else if (result.error?.includes("Cannot bless"))
                statusCode = 400;
            return c.json({
                success: false,
                error: result.error,
...
blessings.post("/prepare-delegate", withAuth, async (c) => {
    try {
        ...
        const result = await blessingService.prepareDelegateApprovalTransaction(user.walletAddress, approved);
        if (!result.success) {
            return c.json({
                success: false,
                error: result.error,
