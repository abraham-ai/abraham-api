...
describe("TheSeeds", function () {
    ...
    describe("Seed Submission", function () {
        it("Should allow submitting a seed", async function () {
            const tx = await seeds.connect(user1).submitSeed("QmTestHash123", "Test Artwork", "A beautiful test artwork");
            await expect(tx)
                .to.emit(seeds, "SeedSubmitted")
                .withArgs(0, user1.address, "QmTestHash123", "Test Artwork", await ethers.provider.getBlock("latest").then(b => b.timestamp));
        ...
        it("Should revert if IPFS hash is empty", async function () {
            await expect(seeds.connect(user1).submitSeed("", "Title", "Description")).to.be.revertedWithCustomError(seeds, "InvalidSeedData");
        });
        it("Should revert if title is empty", async function () {
            await expect(seeds.connect(user1).submitSeed("QmHash", "", "Description")).to.be.revertedWithCustomError(seeds, "InvalidSeedData");
        });
    ...
    describe("Seed Retraction", function () {
        ...
        it("Should not allow non-creator to retract", async function () {
            await seeds.connect(user1).submitSeed("QmHash", "Title", "Description");
            await expect(seeds.connect(user2).retractSeed(0)).to.be.revertedWithCustomError(seeds, "NotSeedCreator");
        });
        it("Should not allow retracting nonexistent seed", async function () {
            await expect(seeds.connect(user1).retractSeed(999)).to.be.revertedWithCustomError(seeds, "SeedNotFound");
        });
    });
    describe("Ownership Root Management", function () {
        ...
        it("Should not allow non-owner to update root", async function () {
            const newRoot = ethers.keccak256(ethers.toUtf8Bytes("new_root"));
            await expect(seeds.connect(user1).updateOwnershipRoot(newRoot)).to.be.reverted;
        });
        it("Should not allow zero root", async function () {
            await expect(seeds.updateOwnershipRoot(ethers.ZeroHash)).to.be.revertedWithCustomError(seeds, "InvalidOwnershipRoot");
        });
    ...
    describe("Pause Functionality", function () {
        ...
        it("Should prevent seed submission when paused", async function () {
            await seeds.pause();
            await expect(seeds.connect(user1).submitSeed("QmHash", "Title", "Description")).to.be.revertedWithCustomError(seeds, "ContractPaused");
        });
        it("Should not allow non-owner to pause", async function () {
            await expect(seeds.connect(user1).pause()).to.be.reverted;
        });
    });
    ...
    describe("Winner Selection", function () {
        it("Should not allow selection before period ends", async function () {
            await seeds.connect(user1).submitSeed("QmHash", "Title", "Description");
            await expect(seeds.selectDailyWinner()).to.be.revertedWithCustomError(seeds, "VotingPeriodNotEnded");
        });
        it("Should revert if no valid winner", async function () {
            // Advance time past voting period
            await ethers.provider.send("evm_increaseTime", [86400]);
            await ethers.provider.send("evm_mine", []);
            await expect(seeds.selectDailyWinner()).to.be.revertedWithCustomError(seeds, "NoValidWinner");
        });
    });
