...
async function waitForContract(
  ...
  console.log("⏳ Waiting for contract to be ready for reads...");
...
function updateAbiTypescriptFile(contractAddress: string): void {
  ...
  if (!existsSync(abiJsonPath)) {
    console.log("⚠️  ABI JSON not found, skipping TypeScript ABI update");
    return;
  }
...
async function main() {
  ...
  console.log(`✅ Merkle root updated on contract`);
  ...
  if (hasRole) {
    console.log("✅ Relayer already has CREATOR_ROLE");
  } else {
    ...
    console.log(`✅ CREATOR_ROLE granted to relayer`);
  ...
  try {
    ...
    console.log(`✅ Test seed created with ID: ${result.testSeedId}`);
  } catch (error: any) {
    console.error("\n❌ Failed to create test seed");
